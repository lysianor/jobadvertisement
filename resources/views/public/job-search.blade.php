                ...
                <form action="{{ url('job-search') }}" method="get">
                  <div class="row">
                    <div class="col-md-4 col-sm-6 col-xs-12 mb-3 mb-md-0">
                      <input name="search" placeholder="Search Keywords" type="text">
                    </div>
    ...
    <div class="container">
      <div class="row mb40">
        <div class="col-lg-8 col-md-12 col-sm-12 col-xs-12">
          ...
          @endforeach
          @else
            <h4 class="mb40 mt-0">There are no jobs matching for your search criteria.</h4>
          @endif
                  ...
                  <div class="swiper-wrapper">
                    ...
                    <div class="swiper-slide">
                      <div class="ui-card">
                        <div class="ui-card-content">
                          <div class="vacancies-title-location">
                            <a href="{{ route('employer.company',[$randompost->id, str_slug($randompost->company)]) }}" class="vacancies-title h6">{{ ucwords($randompost->company) }}</a>
                            <div class="vacancies-location">
                              {{ $randompost->size }}
                            </div>
                          </div>
                          <a href="{{ route('employer.company',[$randompost->id, str_slug($randompost->company)]) }}" class="logo-company w-100">
                            <img src="/profile/{{$randompost->image}}">
                          </a>
                        </div>
                        <div class="ui-card-footer">
                          <span class="link--uppercase-wide fs-12">{{ ucwords($randompost->industry) }}</span>
                        </div>
                      </div>
                    </div>
                    @endforeach
...
<script defer>
  ...
  google.maps.event.addDomListener(window, 'load', initialize);
    ...
    function generateContent(place)
    {
        ...
        //                                             <a href="{{ route('post.details', '') }}/`+place.id+``+place.slug+`" title="View: `+place.title+`">`+place.title+`</a>
        //                                         </h4>
        //                                     </div>
        ...
        //                     <span class="geodir_post_meta_title">Job Title: </span></span>`+place.title+`</div>
        //                 <div class="geodir_post_meta  geodir-field-post_title"><span class="geodir_post_meta_icon geodir-i-text">
        //                     <i class="fa fa-money" aria-hidden="true"></i>
        //                     <span class="geodir_post_meta_title">Salary: </span></span>`+place.salary+`/`+place.per+`</div>
        ...
        var content = `
              <div class="leaflet-popup-content-wrapper">
                <div class="leaflet-popup-content" style="width: 179px;">
                  <h6><a href="{{ route('post.details', '') }}/`+place.id+``+place.slug+`" title="View: `+place.title+`">`+place.title+`</a></h6>
                  <div class="vacancies-location">&#8369; `+place.salary+` / `+place.per+`</div> 
